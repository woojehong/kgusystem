  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      <button type="button" onClick={() => onChange('all')}
        className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition ${isAll ? 'border-indigo-400 bg-indigo-500/15 text-indigo-200' : 'border-base-700 text-base-400 hover:text-base-200'}`}>
        전부
      </button>
      {subCategories.map((sc) => (
        <button key={sc.id} type="button" onClick={() => toggle(sc.id)}
          className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition ${!isAll && arr.includes(sc.id) ? 'border-indigo-400 bg-indigo-500/15 text-indigo-200' : 'border-base-700 text-base-400 hover:text-base-200'}`}>
          {sc.label}
        </button>
      ))}
    </div>
...
// 길드 마스터 전용: 이 길드 레이드를 받을 디스코드 채널 관리.
  ...
  const addChannel = async () => {
    ...
    try {
      await setDoc(doc(db, 'cardChannels', cid), {
        channelId: cid, filter: guildFilter, subFilter: newSub, createdByGuild: guild.id, enabled: true, updatedAt: Date.now(),
      });
      setNewId(''); setNewSub('all'); setMsg({ ok: true, text: '채널을 등록했어요.' });
    } catch {
    ...
    <div className="card p-4 space-y-4">
      <div>
        <h3 className="font-bold text-base flex items-center gap-2">
          <span>🔔 디스코드 알림 채널</span>
          <span className="text-[11px] px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-300 font-semibold">마스터</span>
        </h3>
        <p className="text-xs text-base-400 mt-1">이 길드({guild.name}) 레이드를 받을 디스코드 채널을 관리합니다.</p>
      </div>
      ...
      <div className="rounded-xl border border-base-700 bg-base-850 p-3 space-y-2">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition"
        >
          🤖 길레봇 초대하기
        </a>
      ...
      <div className="rounded-xl border border-base-700 bg-base-850 p-3 space-y-2">
        ...
        <div className="space-y-2">
          <input
            ...
            placeholder="채널 ID (숫자) — 채널 우클릭 → ID 복사"
            value={newId}
          ...
          <button
            ...
            onClick={addChannel}
            disabled={busy}
            className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition disabled:opacity-50"
          >
            {busy ? '등록 중…' : '이 채널 등록'}
          </button>
        </div>
          ...
          <div className="space-y-2">
            {channels.map((c) => (
              <div key={c.id} className="rounded-xl border border-base-700 bg-base-850 p-2.5">
                ...
                <p className="text-[11px] text-base-500 mt-1 mb-1.5">받는 소분류: <b className="text-base-300">{subLabel(c.subFilter)}</b></p>
